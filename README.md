...
Telegram tak benarkan bot senaraikan ahli group secara automatik.
...
Pilih status + kongsi lokasi berlaku dalam DM peribadi dengan bot,
...
Bot tak boleh mulakan DM dengan sesiapa yang belum pernah mesej bot
...
Geolocation API (cell tower/WiFi) — DIPUTUSKAN TIDAK DIGUNAKAN.
...
DM peribadi dengan bot hanya untuk urusan kehadiran rasmi. Kalau
...
bukan chatbot umum.
"Pembantu admin" (report delegate) boleh dilantik untuk jana laporan
...
Dalam Telegram, cari @BotFather dan mulakan chat.
Hantar `/newbot`, ikut arahan (nama bot & username, contoh `HelmiCheckinBot`).
BotFather akan bagi satu token (bentuk `123456:ABC-DEF...`). Simpan token ini.
Penting - disable privacy mode supaya bot boleh baca mesej lokasi dan
command dalam group:
Hantar `/mybots` kepada BotFather → pilih bot anda → Bot Settings
→ Group Privacy → Turn off.
Masukkan bot ke dalam group Telegram yang anda mahu guna.
2. Dapatkan Google Maps API Key (untuk nama tempat)
Pergi ke Google Cloud Console.
Cipta project baharu (atau guna sedia ada) dan aktifkan billing.
APIs & Services → Library → cari & Enable "Geocoding API".
APIs & Services → Credentials → cipta API key baharu.
(Disyorkan) Sekat API key supaya hanya boleh guna Geocoding API.
3. Dapatkan Gmail App Password (untuk emel laporan)
Aktifkan 2-Step Verification pada akaun Gmail: myaccount.google.com/security.
Pergi ke myaccount.google.com/apppasswords.
Cipta App Password baharu → dapat kod 16-aksara.
`SMTP_USER` = alamat Gmail penuh; `SMTP_PASSWORD` = kod 16-aksara tadi
...
Dalam project Railway yang sama dengan bot ini, klik New → Database
→ Add PostgreSQL.
Railway akan cipta servis Postgres dengan `DATABASE_URL` sendiri.
Dalam servis bot (bukan servis Postgres), pergi ke tab Variables,
...
Setiap ahli (dan admin) DM bot secara peribadi (cari username bot,
klik Start).
Hantar `/daftar Nama Penuh Anda` — contoh: `/daftar Ahmad bin Ali`.
Admin juga hantar `/adminid` dalam DM tersebut, salin User ID yang
...
Push kod ini ke satu repo GitHub (jangan sertakan `.env` sebenar
— `.env.example` sahaja yang perlu dalam repo).
Di Railway, cipta New Project → Deploy from GitHub repo.
Tambah database Postgres dalam project yang sama (lihat Langkah 4 di atas)
kalau belum buat lagi.
Railway detect `requirements.txt` + `Procfile` dan run sebagai worker
(bot guna polling, bukan webhook — tak perlu port/HTTP).
Dalam tab Variables servis bot, tambah SEMUA env var dalam
...
Deploy. Semak Deploy Logs — patut nampak `Bot bermula (polling)...`.
